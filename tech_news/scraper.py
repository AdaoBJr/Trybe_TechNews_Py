...
def get_tech_news(amount):
    ...
    while len(arr) < amount:
        arr_html_content = fetch(link)
        link = scrape_next_page_link(arr_html_content)
        arr_urls = scrape_novidades(arr_html_content)
        arr.extend(arr_urls)
    arr = arr[0:amount]
    new_arr = [
        scrape_noticia(fetch(val)) for val in arr
    ]
    create_news(new_arr)
    return new_arr
